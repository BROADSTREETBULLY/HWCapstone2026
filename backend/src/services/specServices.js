const { Spec, User } = require("../models");
...
async function getLibrary({ paginationModel, filterModel, sortModel, userId }) {
  const user = await User.findById(userId);
  if (!user) throw new Error("User not found");

  const queryLibrary = filter(filterModel);
  queryLibrary.orgId = user.orgId;
  queryLibrary.ownerType = "library";
  queryLibrary.libraryID = null;
  const sortLibrary = sort(sortModel);
  const { page, pageSize } = paginationModel;

  const [items, itemCount] = await Promise.all([
    Spec.find(queryLibrary)
      .sort(sortLibrary)
      .skip(page * pageSize)
      .limit(pageSize),
